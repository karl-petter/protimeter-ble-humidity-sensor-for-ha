...
```text
Offset  Length  Field
  0       4     Unused / padding (not read by the official app)
...
**Note:** The S response contains no device timestamp. The timestamp shown in the
official app when a live reading is taken is generated by the phone at the moment
the response is received, not by the device.
